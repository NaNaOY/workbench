...
工作台使用本地图片构成低干扰的视觉层级，断网时也可正常显示：

- `public/assets/page-bg-2.jpg`：全局工作区的薄荷渐变背景。
- `public/assets/page-bg-1.jpg`：认知提升标题区的暖色花卉背景。
- 内容卡片使用半透明表面、模糊和轻微饱和度增强形成玻璃质感，同时保持纯净、低干扰的正文背景。
- 页面进入、卡片悬停、按钮按压、下拉菜单和知识抽屉使用短时柔和动效；系统开启“减少动态效果”时会自动关闭大部分动画。
...
全局点击会在指针位置显示约 24px 的微型光环与六颗双色光点，效果不捕获事件、不影响按钮操作，并在系统开启“减少动态效果”时自动停用。

主题覆盖集中维护在 `src/glass-theme.css`。`src/main.tsx` 将它放在基础样式和素材主题之后载入；`src/App.tsx` 会给工作区附加当前页面类名，便于按模块扩展背景而不影响业务组件。
